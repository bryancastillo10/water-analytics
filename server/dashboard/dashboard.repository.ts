...
import {
  ...
  validateCalcData,
  validateStatType,
} from '@/dashboard/utils/statTypes';
...
export class DashboardRepository implements IDashboardRepository {
  ...
  async getParameterFilters(userId: string): Promise<string[]> {
    try {
      ...
      const paramtersList = parameters.map(
        param => parameterRecord[param.parameter] || param.parameter,
      );
  ...
  async timeSeries({
    ...
    startDate,
    endDate,
  }: GetTimeSeriesDataRequest): Promise<TimeSeriesData[]> {
    try {
  ...
  async getParameterProfile(siteId: string, parameters: string[]): Promise<IParameterProfile> {
    try {
      ...
      const avgFields = parameters.reduce(
        (acc, param) => {
          acc[param] = true;
          return acc;
        },
