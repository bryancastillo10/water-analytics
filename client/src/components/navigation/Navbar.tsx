...
const Navbar = ({ isSidebarExpanded, theme, toggleSidebar, toggleTheme }: NavigationProps) => {
  ...
  const currentPage = sidebarItems.find(item => item.link === normalizedPathname)?.name || 'Users';
    ...
    <nav className="sticky z-20 top-0 bg-primary text-light px-6 py-4">
      <section className="flex items-center justify-between">
          ...
          {isSidebarExpanded && (
            <>
              <div
                ...
                className="cursor-pointer md:hidden hover:scale-90 duration-150"
              >
                <List size="24" />
              </div>
            </>
        ...
        <div className="flex items-center gap-x-3">
          ...
          <div className="relative flex items-center gap-x-3" ref={popoverRef}>
            {/* Avatar */}
            <img
