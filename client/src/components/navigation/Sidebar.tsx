...
const SideBarRowItem = ({
  ...
  return (
    <Link to={`/${role}${link}`}>
      <li
        className={`${isActive ? 'bg-primary' : null} 
      ...
      >
        ...
        {isSidebarExpanded ? null : (
          <span
            className={`${isActive ? 'text-white font-semibold' : theme ? 'text-secondary' : 'text-dark'}`}
          >
            {name}
          </span>
        )}
      ...
      <div className={`relative py-8 ${isSidebarExpanded ? 'px-2' : 'px-4'} flex-grow`}>
        ...
        <div className="flex justify-start gap-2 items-center">
          <img
            src={AppLogo}
            alt="logo"
            className={`${isSidebarExpanded ? 'mt-8' : 'mt-0'} size-10`}
          />
          {!isSidebarExpanded && (
