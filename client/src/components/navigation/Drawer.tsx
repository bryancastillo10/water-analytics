...
const Drawer = () => {
  ...
  return (
    ...
    <div className={`fixed z-40 flex flex-col top-0 right-0 w-full xl:w-[50%] h-screen 
     ...
     `}>
            <div className={`flex justify-between items-center text-primary border-b py-2 ${theme ? "border-secondary":"border-neutral"}`}>
              <h1 className={`text-2xl tracking-wider ${theme ? "text-secondary":"text-primary"}`}>{title}</h1>
              <div onClick={onClose} className="rounded-full cursor-pointer hover:bg-primary hover:text-light duration-500 p-2">
                  <X size="20" /> 
              </div>
            </div>
