...
const StatisticsProfileCharts = () => {
  ...
  const { selectedLabel, selectedValue, paramGroupOptions, selectParameterGroup } =
    useParameterGroupSelection();
  ...
  const {
    ...
    { siteId: safeSiteId, paramgroup: selectedValue },
    { skip: !siteId },
  );
  ...
  const gaugeData =
    selectedValue === 'basic'
      ? statsData.filter(
          param => param.parameter !== 'pH' && param.parameter.toLowerCase() !== 'temperature',
        )
      : statsData;
