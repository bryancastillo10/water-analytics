...
const DashboardCard = ({
  ...
  const roundedValue = value != null ? Math.round(value * 10) / 10 : 'N/A';

  return (
    <article
      className="w-full h-[200px] overflow-hidden max-w-md bg-gradient-to-br 
      ...
      <div className="relative flex justify-evenly items-center gap-4">
        ...
        <div className="flex flex-col items-start">
          <div className="flex flex-col items-baseline gap-1 mb-2">
            <span className="text-4xl font-bold tracking-tight">{roundedValue}</span>
            <span className={`text-lg font-medium ${unit === '' ? 'my-3' : 'my-0'}`}>{unit}</span>
          </div>
          <div className={`flex items-center gap-x-2 ${colorClass}`}>
            {<TrendIcon className="size-6" />}
            <span className="text-xs lg:text-sm font-semibold whitespace-nowrap">{status}</span>
          </div>
        </div>
      </div>
    </article>
