...
const DeleteMeasurementData = ({ id, data }: DeleteMeasurementDataProps) => {
  ...
  if (!findMeasurement) {
    return <div className="text-lg"> No Measurement Was Found with the ID</div>;
  }
        ...
        <>
          ...
          <div className="grid grid-cols-2 items-center mb-2">
            <h1 className="flex items-center gap-x-2">
              <CalendarBlank size="20" /> Sampling Date
            </h1>
          ...
          <BasicParamsTableView paramsData={basicParamsData} />
          <FormSubheader icon={Hexagon} text="Organic Pollution Indicators" />
          <OrgIndParamsTableView paramsData={orgIndParamsData} />
          <FormSubheader icon={Plant} text="Nutrient Pollution Indicators" />
          <NutrientParamsTableView paramsData={nutrientParamsData} />{' '}
        </>
      ) : (
        <DrawerLoadingState />
      )}
