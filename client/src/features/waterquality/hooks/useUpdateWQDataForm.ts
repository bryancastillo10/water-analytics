...
const useUpdateWQData = (findMeasurement: IMeasurementData) => {
  ...
  // States
  const [sampleDate, setSampleDate] = useState<Date | null>(
    findMeasurement.date ? new Date(findMeasurement.date) : null,
  );
  const [basicParamsData, setBasicParamsData] = useState<IBasicParams>(
    basicParamsToUpdate as IBasicParams,
  );
  const [orgIndParamsData, setOrgIndParamsData] = useState<IOrgIndicatorParams>(
    orgIndParamsToUpdate as IOrgIndicatorParams,
  );
  const [nutrientParamsData, setNutrientParamsData] = useState<INutrientParams>(
    nutrientParamsToUpdate as INutrientParams,
  );
  ...
  const onChangeInput =
    <T extends IBasicParams | IOrgIndicatorParams | INutrientParams>(
